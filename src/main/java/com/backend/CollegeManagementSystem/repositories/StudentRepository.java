...
public interface StudentRepository extends JpaRepository<StudentEntity, Long> {
    ...
    @Query("""
                   SELECT new com.backend.CollegeManagementSystem.dtos.flat.StudentFlatDto(
                       s.id, s.name, s.registrationNumber, sub.title, p.title
                   )
    ...
    /*
     * returns all students complete entity by initializing relationships eagerly
     */
    @Query("""
                   SELECT DISTINCT s
                   FROM StudentEntity s
            ...
            """)
    List<StudentEntity> findAllStudentsWithRelations();
    ...
    List<StudentEntity> findStudentsByNameWithRelations(@Param("name") String name);

    /*
     * return the student entity with given registration number
     */
    StudentEntity findStudentByRegistrationNumber(String registrationNumber);
